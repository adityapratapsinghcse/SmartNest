...
export function AuthProvider({ children }) {
  ...
  const [householdId, setHouseholdId] = useState(() => localStorage.getItem('smartnest_household_id'));
  const [householdName, setHouseholdName] = useState(() => localStorage.getItem('smartnest_household_name'));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // On app load, if we have a token but no household yet, fetch it
    if (token && !householdId) {
      client.get('/api/auth/my-households/')
        .then((res) => {
  ...
  const register = async (username, email, password, householdName) => {
    const res = await client.post('/api/auth/register/', {
      username, email, password, household_name: householdName,
    });
